...
''' knn 알고리즘 객체 생성  k=10 (default) '''
knn = cv2.ml.KNearest_create()


def start(sample_size=25):
    # sample_size(25)만큼의 train data를  generate_data 함수를 통해 생성
    train_data = generate_data(sample_size)
    # print("train_data :",train_data)
    # classify_label 함수를 통해 train_data들의 label들을 설정해 준다.
...
# 시간을 기준으로 risk-level(0,1,2)을 정한다.
# 내 생각에는 눈 감은 시간 index 0 눈 뜬 시간 index 1 을 기준으로 labels 0,1,2 로 나누는 것 같아.
...
'''
2 short : 졸음 강도 약함 (label이 2인 data를 뽑아서 short에 집어넣기/가져옴)
1 normal : 졸음 강도 중간 (label이 1인 data를 뽑아서 normal에 집어넣기)
0 power : 졸음 강도 강함 (label이 0인 data를 뽑아서 power에 집어넣기)
train data에서 label에 따라 3가지로 분류
'''
...
''' 이 부분은 정확도 측정 및 디버깅 해보려고 넣은 부분인 거 같아.'''
